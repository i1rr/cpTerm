...
/// (section_name, field_label) for each of the 26 theme fields, in order.
...
pub fn color_display_name(c: Color) -> String {
    match c {
        ...
        Color::Indexed(n) if (16..232).contains(&n) => {
            let v = n - 16;
            let r = v / 36;
            let g = (v % 36) / 6;
            let b = v % 6;
...
impl Theme {
    ...
    pub fn get_field(&self, index: usize) -> Color {
        match index {
            ...
            16 => self.input_fg,
            17 => self.help_fg,
            18 => self.confirm_border,
            19 => self.confirm_fg,
            20 => self.conflict_border,
            21 => self.conflict_fg,
            22 => self.error_border,
            23 => self.error_fg,
            24 => self.info_border,
            25 => self.info_fg,
            _ => Color::Reset,
    ...
    pub fn set_field(&mut self, index: usize, color: Color) {
        match index {
            ...
            5 => self.new_file_fg = color,
            6 => self.status_bg = color,
            7 => self.status_fg = color,
            8 => self.path_fg = color,
            9 => self.selection_fg = color,
            10 => self.filter_fg = color,
            11 => self.cmdbar_bg = color,
            12 => self.cmdbar_fg = color,
            13 => self.fkey_fg = color,
            14 => self.fkey_bg = color,
            15 => self.hint_fg = color,
            16 => self.input_fg = color,
            17 => self.help_fg = color,
            18 => self.confirm_border = color,
            19 => self.confirm_fg = color,
            20 => self.conflict_border = color,
            21 => self.conflict_fg = color,
            22 => self.error_border = color,
            23 => self.error_fg = color,
            24 => self.info_border = color,
            25 => self.info_fg = color,
            _ => {}
        }
    }
