...
use super::remote_explorer::RemoteExplorer;

/// Contents of a single pane slot - either a file explorer, an embedded editor, or a remote explorer.
...
impl PaneContent {
    fn refresh(&mut self) {
        match self {
            PaneContent::Explorer(e) => e.refresh(),
            PaneContent::Editor(_) => {}
            PaneContent::Remote(_) => {} // Remote refresh is async - handled by app
        }
    }
    ...
    pub fn current_dir(&self) -> PathBuf {
        match self {
            ...
            PaneContent::Remote(r) => PathBuf::from(&r.current_path),
        }
    }
}
...
impl DualPane {
    ...
    pub fn active_remote(&self) -> Option<&RemoteExplorer> {
        match self.active_content() {
            PaneContent::Remote(r) => Some(r),
            _ => None,
        }
    }

    pub fn active_remote_mut(&mut self) -> Option<&mut RemoteExplorer> {
        match self.active_content_mut() {
            PaneContent::Remote(r) => Some(r),
            _ => None,
        }
    ...
    pub fn inactive_remote(&self) -> Option<&RemoteExplorer> {
        match self.inactive_content() {
            PaneContent::Remote(r) => Some(r),
            _ => None,
        }
    }
    ...
    pub fn snapshot_dir(&self, dir: &std::path::Path) -> HashSet<PathBuf> {
        ...
        for pane in [&self.left, &self.right] {
            ...
            // Remote panes are skipped - no local paths to snapshot
        }
        if paths.is_empty() {
    ...
    pub fn close_editor_in_active(&mut self) {
        let origin_dir = match self.active_content() {
            PaneContent::Editor(e) => e.origin_dir.clone(),
            PaneContent::Explorer(_) | PaneContent::Remote(_) => return,
        };
    ...
    pub fn draw(&mut self, frame: &mut Frame, area: Rect, theme: &Theme) {
        if self.editor_fullscreen {
            ...
            match self.active {
                PaneSide::Left => match &mut self.left {
                    ...
                    PaneContent::Remote(r) => r.draw(frame, area, true, theme),
                },
        ...
        match &mut self.left {
            ...
            PaneContent::Remote(r) => r.draw(frame, chunks[0], left_active, theme),
        }
        match &mut self.right {
            ...
            PaneContent::Remote(r) => r.draw(frame, chunks[1], right_active, theme),
        }
    }
}
