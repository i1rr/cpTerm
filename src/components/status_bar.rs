...
pub fn draw_status_bar(
    ...
    connecting_to: Option<&str>,
    theme: &Theme,
) {
    // Connecting to SSH - show indicator regardless of pane type.
    if let Some(host) = connecting_to {
    ...
    // When the active pane is a remote explorer, show the remote path.
    if let Some(remote) = dual_pane.active_remote() {
        let path = format!("{}{}", remote.host_label, remote.current_path);
        let mut spans = vec![Span::styled(
            format!("{}> ", path),
            Style::default().fg(theme.path_fg),
        )];
        let sel_count = remote.selected.len();
        if sel_count > 0 {
        ...
        if let Some(ref filter) = remote.filter_text {
            if sel_count > 0 {
                spans.push(Span::raw(" | "));
            }
            spans.push(Span::styled(
